          ...
          <nav className="flex items-center space-x-2 text-base font-medium">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="rounded-md px-4 py-2 text-foreground/70 transition-colors hover:bg-muted hover:text-foreground"
              >
                {item.name}
              </Link>
            ))}
                ...
                <nav className="flex flex-col gap-2 text-base font-medium">
                  {navigation.map((item) => (
                    <SheetClose asChild key={item.href}>
                      <Link
                        href={item.href}
                        className="flex items-center gap-4 rounded-md p-3 text-foreground/80 hover:bg-muted"
                      >
